...
function renderDispatch() {
  ...
  return `
    ...
    <section class="dispatch-command">
      <div class="dispatch-command__heading">
        <div>
          <span class="dispatch-command__eyebrow">下发统筹台</span>
          <h2>模板与任务下发追踪</h2>
          <p>统一看清总部任务、活动模板和营销模板在各门店的落地进展，快速定位未完成与逾期对象。</p>
        </div>
        <div class="dispatch-command__badge"><span>当前异常</span><strong>${s.taskOverdue}</strong><small>个逾期任务待干预</small></div>
      </div>
      <div class="dispatch-summary-strip">
        <div><span>任务下发</span><strong>${s.taskCount}</strong><small>平均完成率 ${s.taskAvgCompletion}%</small></div>
        <div><span>逾期任务</span><strong>${s.taskOverdue}</strong><small>已超过截止时间</small></div>
        <div><span>活动模板</span><strong>${s.templateCount}</strong><small>${s.templateTotalEnrollments} 条线索</small></div>
        <div><span>营销模板</span><strong>${s.marketingTemplateCount}</strong><small>${s.marketingAdoptedTotal} 家门店采纳</small></div>
      </div>
    </section>
...
function renderResourceSummary(resource: ResourceKey, rows: any[]) {
  const statusCount = (key: string) => rows.filter((item) => String(item.status ?? "") === key).length;
  const sum = (getter: (item: any) => number) => rows.reduce((total, item) => total + getter(item), 0);
  if (resource === "clinics") {
    const active = statusCount("ACTIVE");
    const cities = new Set(rows.map((item) => item.city).filter(Boolean));
    const sales = sum((item) => Number(item.monthlySales ?? 0));
    const avgSales = rows.length ? Math.round(sales / rows.length) : 0;
    return `<section class="resource-summary-strip">
      <div><span>门店规模</span><strong>${rows.length}</strong><small>${active} 家启用中</small></div>
      <div><span>覆盖城市</span><strong>${cities.size}</strong><small>便于区域统筹管理</small></div>
      <div><span>累计产出</span><strong>${money(sales)}</strong><small>店均 ${money(avgSales)}</small></div>
      <div><span>停用门店</span><strong>${rows.length - active}</strong><small>需要复核经营状态</small></div>
    </section>`;
  }
  if (resource === "users") {
    const active = statusCount("ACTIVE");
    const platform = rows.filter((item) => item.role === "PLATFORM_ADMIN").length;
    const clinic = rows.length - platform;
    return `<section class="resource-summary-strip">
      <div><span>账号总量</span><strong>${rows.length}</strong><small>${active} 个正常启用</small></div>
      <div><span>总部账号</span><strong>${platform}</strong><small>用于统筹权限与下发</small></div>
      <div><span>门店账号</span><strong>${clinic}</strong><small>医生与门店协同账号</small></div>
      <div><span>停用账号</span><strong>${rows.length - active}</strong><small>需清理长期不用账号</small></div>
    </section>`;
  }
  if (resource === "kits") {
    const avgRetail = rows.length ? Math.round(sum((item) => Number(item.retailPrice ?? 0)) / rows.length) : 0;
    const avgClinic = rows.length ? Math.round(sum((item) => Number(item.clinicPrice ?? 0)) / rows.length) : 0;
    return `<section class="resource-summary-strip">
      <div><span>产品包数量</span><strong>${rows.length}</strong><small>覆盖不同治疗组合</small></div>
      <div><span>建议零售价</span><strong>${money(avgRetail)}</strong><small>按当前列表均值</small></div>
      <div><span>门店供货价</span><strong>${money(avgClinic)}</strong><small>便于毛利比较</small></div>
      <div><span>停用包</span><strong>${statusCount("DISCONTINUED")}</strong><small>建议及时归档整理</small></div>
    </section>`;
  }
  if (["templates", "articles", "marketingPosts", "taskTemplates"].includes(resource)) {
    return `<section class="resource-summary-strip">
      <div><span>内容总量</span><strong>${rows.length}</strong><small>${configLabel(resource)} 内容资产</small></div>
      <div><span>已发布</span><strong>${statusCount("PUBLISHED")}</strong><small>当前可被门店采纳</small></div>
      <div><span>草稿中</span><strong>${statusCount("DRAFT")}</strong><small>待补充与审核</small></div>
      <div><span>已归档</span><strong>${statusCount("ARCHIVED")}</strong><small>沉淀历史版本</small></div>
    </section>`;
  }
  if (resource === "tasks") {
    const done = statusCount("DONE");
    return `<section class="resource-summary-strip">
      <div><span>任务总量</span><strong>${rows.length}</strong><small>总部统一运营任务</small></div>
      <div><span>已完成</span><strong>${done}</strong><small>${rows.length ? Math.round((done / rows.length) * 100) : 0}% 完成率</small></div>
      <div><span>待推进</span><strong>${rows.length - done}</strong><small>建议优先跟进逾期任务</small></div>
      <div><span>紧急任务</span><strong>${rows.filter((item) => item.priority === "URGENT").length}</strong><small>需要重点处理</small></div>
    </section>`;
  }
  if (resource === "purchases") {
    const amount = sum((item) => Number(item.totalAmount ?? item.amount ?? 0));
    return `<section class="resource-summary-strip">
      <div><span>订单总量</span><strong>${rows.length}</strong><small>门店采购汇总</small></div>
      <div><span>采购金额</span><strong>${money(amount)}</strong><small>已汇总当前筛选结果</small></div>
      <div><span>已完成</span><strong>${statusCount("COMPLETED")}</strong><small>已确认入账订单</small></div>
      <div><span>待处理</span><strong>${statusCount("PENDING")}</strong><small>等待确认或支付</small></div>
    </section>`;
  }
  return "";
}

function renderResourceInsights(resource: ResourceKey, rows: any[]) {
  if (resource === "clinics") {
    const cityRows = Array.from(rows.reduce((map, item) => {
      const key = String(item.city || "未设置城市");
      map.set(key, (map.get(key) ?? 0) + 1);
      return map;
    }, new Map<string, number>()).entries() as Iterable<[string, number]>)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 6);
    return `<aside class="resource-insight-panel">
      <div class="resource-insight-card">
        <span>城市分布</span>
        <h3>门店覆盖热区</h3>
        <div class="resource-insight-list">
          ${cityRows.map(([city, count]) => `<div><strong>${html(city)}</strong><small>${count} 家门店</small></div>`).join("") || `<div><strong>暂无数据</strong><small>等待门店信息补充</small></div>`}
        </div>
      </div>
    </aside>`;
  }
  if (resource === "kits") {
    const topRows = [...rows]
      .sort((a, b) => Number(b.retailPrice ?? 0) - Number(a.retailPrice ?? 0))
      .slice(0, 5);
    return `<aside class="resource-insight-panel">
      <div class="resource-insight-card">
        <span>价格带观察</span>
        <h3>高零售价产品包</h3>
        <div class="resource-insight-list">
          ${topRows.map((item) => `<div><strong>${html(item.name)}</strong><small>${money(item.retailPrice ?? 0)} · 门店价 ${money(item.clinicPrice ?? 0)}</small></div>`).join("") || `<div><strong>暂无数据</strong><small>当前没有产品包</small></div>`}
        </div>
      </div>
    </aside>`;
  }
  const statusRows = Array.from(rows.reduce((map, item) => {
    const key = statusLabel(String(item.status ?? "UNKNOWN"));
    map.set(key, (map.get(key) ?? 0) + 1);
    return map;
  }, new Map<string, number>()).entries() as Iterable<[string, number]>).sort((a, b) => b[1] - a[1]);
  return `<aside class="resource-insight-panel">
    <div class="resource-insight-card">
      <span>状态结构</span>
      <h3>${configLabel(resource)}分布</h3>
      <div class="resource-insight-list">
        ${statusRows.map(([label, count]) => `<div><strong>${html(label)}</strong><small>${count} 条</small></div>`).join("") || `<div><strong>暂无数据</strong><small>等待内容沉淀</small></div>`}
      </div>
    </div>
  </aside>`;
}

function configLabel(resource: ResourceKey) {
  return configs[resource]?.label ?? resource;
}
...
function renderResource(resource: ResourceKey) {
  ...
  return `
    ...
    <section class="resource-workspace">
      ${renderResourceSummary(resource, rows)}
      <div class="resource-content-grid">
        ...
        ${renderResourceInsights(resource, rows)}
      </div>
